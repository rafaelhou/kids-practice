...
三個遊戲：⏰ 現在幾點？（選 `10 點 35 分`）、🔢 配數字鐘（圓形錶配 `3:30`，用他已經會的數字架不會的）、🔍 哪一個時鐘？（聽中文報時挑錶面）。

**畫面上的時間一律寫阿拉伯數字，唸出來才用中文講法。**
選項寫「八點」等於在考兩件事——先讀懂國字，才輪到看時鐘；他認得數字但國字還在學，中文數字會變成沒必要的門檻。
顯示用 `numTime()`、發音用 `zhTime()`，判題訊息也分開（`judge()` 的 `label` 是看的、`speak` 是唸的）。
...
clock.js        錶面繪製、numTime（顯示）、zhTime（發音）、digitalTime
